...
const { securityConfigs: { JWT_SECRET } } = require("../../configs");
...
const { constants: { userManagementConstants: { userConstants: { userStatus } } }, securityUtils : { hashText } } = require("../../utils");
const { emailMiddlewares: { isValidEmail }, contactNumberMiddlewares: { validatePhoneNumberBasedOnCountryCode, validateCountryCode }, fieldDataMiddlewares: { validateTextField, validateNumberField, } } = require("../common");

const { validateUserStatus } = require("./user_middlewares");
...
const signUpRequestValidation = async (req, res, next) => {
    ...
    req.userCreateObject = {
        ...
        contactNumber,
        status: userStatus.VERIFICATION_PENDING ? "VERIFICATION_PENDING" : "VERIFICATION_PENDING"
    };

    const fieldsToValidate = [
        {
            field: "firstName",
            value: req.userCreateObject.firstName,
            options: {
                required: true,
                minLength: 1,
                maxLength: 50,
                checkSpaceAtFirstAndLast: true,
                validationFunction: validateTextField, // Set the validation function
            },
        },
        {
            field: "lastName",
            value: req.userCreateObject.lastName,
            options: {
                required: true,
                minLength: 1,
                maxLength: 50,
        ...
        {
            field: "email",
            value: req.userCreateObject.email,
            options: {
                required: true,
                checkLowerCase: true,
                customValidation: (value) => {isValidEmail(value)},
                checkSpaceAtFirstAndLast: true,
                validationFunction: validateTextField, // Set the validation function
            },
        },
        {
            ...
            value: req.userCreateObject.authentication.password,
            options: {
                required: true,
                customValidation: (value) => { verifyPasswordRequirements (value) },
                checkSpaceAtFirstAndLast: true,
                validationFunction: validateTextField, // Set the validation function
            },
        },
        {
            field: "contactNumber.primaryCountryCode",
            value: req.userCreateObject?.contactNumber?.primaryCountryCode,
            options: {
                required: true,
                checkSpaceAtFirstAndLast: true,
                customValidation: (value) => {validateCountryCode (value)},
                validationFunction: validateTextField, // Set the validation function
            },
        },
        {
            field: "contactNumber.primaryPhoneNumber",
            value: req.userCreateObject?.contactNumber?.primaryPhoneNumber,
            options: {
                required: true,
                customValidation: (value) => {validatePhoneNumberBasedOnCountryCode(
                    value,
                    req.userCreateObject?.contactNumber?.primaryCountryCode
                )
            },
                validationFunction: validateNumberField, // Set the validation function
            },
        },
        {
            field: "status",
            value: req.userCreateObject.status,
            options: {
                required: true,
                customValidation: (value) => {validateUserStatus(value)},
                validationFunction: validateTextField, // Set the validation function
            },
        },
    ];

    for (let { field, options, value } of fieldsToValidate) {
        const { validationFunction, ...validationOptions } = options; // Destructure to get validation function

        const validatedField = await validationFunction(Users, field, validationOptions, value); // Call the appropriate validation function
        if (validatedField && validatedField.error) {
            return errorResponse(res, validatedField.error.message, 400, validatedField.error.errorInfo);
        } else if (validatedField && validatedField.value) {
            req.userCreateObject[field] = validatedField.value; // Update the req.userCreateObject
        }
    }
...
const hashPasswordAfterValidation = async (req, res, next) => {
    try {
        const hashedPassword = await hashText(req.userCreateObject.authentication.password);
        req.userCreateObject.authentication.password = hashedPassword;
        next();
    } catch (error) {
        throw error;
    }
...
//         { fieldPath: "req.userCreateObject.status", field: "status", value: req.userCreateObject.status, options: { required: true, customValidation: validateUserStatus } },
//     ];
